import admin from 'firebase-admin';

export default async function handler(req, res) {
  // 1. Tarayıcı Güvenlik (CORS) Ayarları
  res.setHeader('Access-Control-Allow-Credentials', true);
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'OPTIONS,POST');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');

  // Tarayıcı deneme isteğine (Preflight) onay ver
  if (req.method === 'OPTIONS') {
    return res.status(200).end();
  }

  try {
    // 2. Firebase Admin Başlatma
    if (!admin.apps.length) {
      // Satır atlamalarının (Newlines) Vercel'de bozulmasını kesin olarak engelliyoruz
      const privateKey = "-----BEGIN PRIVATE KEY-----\\nNTkLpt8mm5JwPgnhtDTQ7g1Q\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n');

      admin.initializeApp({
        credential: admin.credential.cert({
          projectId: "halisaha-app-10dff",
          clientEmail: "firebase-adminsdk-fbsvc@halisaha-app-10dff.iam.gserviceaccount.com",
          privateKey: privateKey,
        }),
      });
    }

    // 3. İstek Metodu Kontrolü
    if (req.method !== 'POST') {
      return res.status(405).json({ error: 'Sadece POST metodu desteklenir.' });
    }

    const { title, body, tokens } = req.body;

    if (!tokens || tokens.length === 0) {
      return res.status(400).json({ error: 'Gönderilecek cihaz (FCM token) bulunamadı.' });
    }

    // 4. Bildirimi Gönder
    const message = {
      notification: { title, body },
      tokens: tokens,
    };

    const response = await admin.messaging().sendEachForMulticast(message);
    return res.status(200).json({ success: true, response });

  } catch (error) {
    console.error('Push Bildirim Hatası Detayı:', error);
    // Vercel çökerse artık sebebi JSON formatında ekrana yansıyacak
    return res.status(500).json({ error: 'Sunucu İç Hatası (Firebase Admin)', details: error.message });
  }
}
